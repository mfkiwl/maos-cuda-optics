...
Real curecon_t::fit(dcell **_dmfit, dcell *_opdr){
    ...
    add2cpu(_dmfit, 0, dmfit, 1, cgstream);
    cgstream.sync();
    return cgres;
}
...
void curecon_t::fit_test(SIM_T *simu){	//Debugging. 
    ...
    dcell *lc=NULL;	
    if(!simu->opdr && opdr){
	cp2cpu(&simu->opdr, opdr, 0);
    }
    if(!simu->parms->gpu.tomo && simu->opdr){
	cp2gpu(opdr, simu->opdr);
    }
