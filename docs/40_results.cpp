...
or cle=read('Res_1.bin.gz');
   ...
   If there is any problem in compling read.cpp and write.cpp, there are two matlab
   scripts in the scripts folder, readbin.m and writebin.m, that have most of
   functionality of read.cpp, write.cpp albeit a bit slower and cannot handle
